...
def fine_tune(data: dict):
  ...
  command_list = [
    ...
    f"tmux send-keys -t fine_tune_session 'sudo docker run --gpus all -e WANDB_API_KEY={wandb_api_key} -e BUCKET_NAME={BUCKET_NAME} -e NEW_DIR_NAME={NEW_DIR_NAME} -e ACCESS_KEY_ID={ACCESS_KEY_ID} -e SECRET_ACCESS_KEY={SECRET_ACCESS_KEY} -v $(pwd)/prompt_dir/:/prompt_dir/ ratna1sambhav/ai_tuners_axolotl_ft:0.1' Enter",
    ]
